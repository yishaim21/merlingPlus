...
class SMSNotifier_slng_Provider implements SMSNotifier_ISMSProvider_Model {
	...
	const SERVICE_URI = 'http://www.slng5.com/WebService/SendSms.asmx';
	private static $REQUIRED_PARAMETERS = array('SourceNumber');
	...
	public function getServiceURL($type = false) {
		if($type) {
			switch(strtoupper($type)) {
				...
				case self::SERVICE_SEND:	return self::SERVICE_URI . '/CreateAndSendSmsToMultiContactsByMobileNo';
	 ...
	 */
	public function send($message, $sourceNumber, $toNumbers) {
		...
		if ($sourceNumber == null || $sourceNumber == "")
			$params['FromMobile'] = $this->getParameter('SourceNumber');
		else
			$params['FromMobile'] = $sourceNumber;
		$params['GlobalID'] = '-1';
		$params['MsgName'] = 'FromVtiger';
