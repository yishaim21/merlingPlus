...
class SMSNotifier extends SMSNotifierBase {
	 ...
	 */
	static function sendsms($message,$sourceNumber, $tonumbers, $ownerid = false, $linktoids = false, $linktoModule = false) {
		...
		// TODO: Save also $sourceNumber to record.
		...
		$responses = self::fireSendSMS($message, $sourceNumber,  $tonumbers);
	...
	static function fireSendSMS($message, $sourceNumber, $tonumbers) {
		$log = vglobal('log');
		$provider = SMSNotifierManager::getActiveProviderInstance();
		if($provider) {
			return $provider->send($message, $sourceNumber, $tonumbers);
		}
	}
